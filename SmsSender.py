...
def sms_sender():
        res = {
               "message": "Hello World",
               "destinationAddresses": "tel:94773887502", # Use the number, in format received from ideamart
               "password": "@ESsW2SpbHbzLxi",  # Use your app password
               "applicationId": "Beta999" # Use your app ID
               }
        with open("SMSMT.txt", "w") as text_file:
            print("send message: {}".format(res), file=text_file)
        url = "https://api.dialog.lk/sms/send" # Use production API in the live environment
